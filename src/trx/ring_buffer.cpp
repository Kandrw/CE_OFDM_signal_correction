...
int RingSamples::Pop(int (*get_data)(const void *, size_t)) {
    ...
    get_data(data_ptr + sizeof(phy_buffer), msg_phy->size / sizeof(mod_symbol));
    msg_phy->size = 0;
    down_index();
    unlock();
    return 1;
...
int RingSamples::Push(int (*get_data)(void *, size_t)) {
    ...
    data_ptr = get_pointer_push();
    phy_buffer *msg_phy = (phy_buffer*)data_ptr;
    get_data(data_ptr + sizeof(phy_buffer), (get_size_element() - sizeof(phy_buffer)) / sizeof(mod_symbol));
